...
class DANNModel(nn.Module):
    def __init__(self):
        ...
        self.feature.add_module('f_linear2', nn.Linear(64, 100))
        self.feature.add_module('f_sig', nn.Sigmoid())

        self.class_classifier = nn.Sequential()
        self.class_classifier.add_module('c_linear1', nn.Linear(100, 64))
        # self.class_classifier.add_module('c_bn1', nn.BatchNorm1d(64))
        self.class_classifier.add_module('c_relu1', nn.LeakyReLU(0.02))
        self.class_classifier.add_module('c_linear2', nn.Linear(64, 1))
        self.class_classifier.add_module('c_sig', nn.Sigmoid())

        self.domain_classifier = nn.Sequential()
        self.domain_classifier.add_module('d_linear1', nn.Linear(100, 64))
...
optimizer = optim.Adam(my_net.parameters(), lr=0.00003)
...
for epoch in range(n_epoch):
    for i in range(165):
        ...
        optimizer.step()
        if (i % 10 == 0):
            print('原数据预测模型loss：',err_s_label,'===','原数据鉴别模型loss：',err_s_domain)
            print('目标数据鉴别模型loss：',err_t_domain)
    # xx = np.reshape(xx,(165, 1)).cpu()
